	...
	try {
		...
		const sanitizeBoolean = (
			...
			if (typeof value !== "boolean") {
				console.debug(
					`Non-boolean value for field: ${fieldName}`,
					value
				)
			}
		...
		return {
			basic: {
				...
				urlFriendlyName: sanitizeString(
					woolworthsProduct.UrlFriendlyName
				),
				description: sanitizeString(
					woolworthsProduct.Description
				).replace(/<br>/g, " "),
				images: {
				...
				minimumQuantity: sanitizeNumber(
					woolworthsProduct.MinimumQuantity
				),
			},
			status: {
			...
			pricing: {
				current: {
					...
					savingsAmount: sanitizeNumber(
						woolworthsProduct.SavingsAmount
					),
